...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
    ...
    try {
        ...
        if (!postItem) return new Response(
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
    try {
        ...
        if (!postItem) {
            return new Response(JSON.stringify({ message: 'No item find for this ID..' }), {
                status: 404
            });
        }
        ...
        return new Response(JSON.stringify({ message: 'Error while delete post..' }), {
            status: 500
        });
    }
}
